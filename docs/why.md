...
`go-ruby-pundit/pundit` reimplements Ruby's `pundit` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-pundit/pundit` directly, with no Ruby runtime;
...
Because `pundit` is pure Go with cgo disabled, it also compiles to **WebAssembly**
